...
const AddScreen = ({navigation, route}) => {
  ...
  // const definition
  const MAX_PEOPLE_LIMIT = 100;

  const handleNumOfPeopleChange = (text) => {
    const parsedNum = parseInt(text);
    if (!isNaN(parsedNum) && parsedNum <= MAX_PEOPLE_LIMIT) {
      setNumOfPeople(parsedNum);
    }
  };
  
  // Chose Image function
  ...
  // Click to Add party Button function
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={styles.inputRow}>
          ...
          <TextInput
            ...
            onChangeText={handleNumOfPeopleChange}
          />
        </View>
